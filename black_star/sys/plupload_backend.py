...
def upload_with_checksum(request, dst, md5chunk, md5total, chunk, chunks):
    ...
    """
    buf_len = int(request.values['chunk_size'])
...
def upload(request):
    ...
    randkey = request.form.get('randkey', 'NONO')
    filename = "%s-%s" % (clean_filename(request.form['name']), randkey)
    dst = os.path.join(UPLOAD_DIR,filename)

    md5chunk = request.form.get('md5chunk', False)
    md5total = request.form.get('md5total', False)
...
def probe(request):
    randkey = request.values.get('randkey', 'NONO')
    filename = "%s-%s" % (clean_filename(request.values['name']), randkey)
